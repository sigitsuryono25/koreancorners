...
class Api extends CI_Controller {
    ...
    function getListKategoriBySource($source) {
        ...
        if ($kategori->num_rows() > 0) {
            foreach ($kategori->result() as $key => $k) {
                ...
                $tmp['parent_kategori'] = $k->parent_kategori;
                $tmp['slug'] = $k->slug;
    ...
    function insertContent() {
        ...
        $originalPublishedDate = $this->input->post('original_published_date');
        if(empty($originalPublishedDate)){
            $originalPublishedDate = date("Y-m-d H:i:s");
        }
